...
const ChatSideBar = ({chats, chatId}: Props) => {
  return (
    <div className='w-full h-screen p-4 text-gray-200 bg-gray-900 flex flex-col'>
        <Link href='/'>
            <Button className='w-full border-dashed border-white border font-poppins'>
            ...
            {chats.map(chat => (
                <Link key={chat.id} href={`/chat/${chat.id}`}>
                    <div className={
                        ...
                        <p className='w-full overflow-hidden text-sm truncate whitespace-nowrap text-ellipsis font-poppins'>{chat.pdfName}</p>
                    </div>
                </Link>
            ))}
        </div>
